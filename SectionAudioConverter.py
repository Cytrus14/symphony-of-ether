    ...
    def InstrumentalConvert(self):
        ...
        while len(secList) != 0:
            for section in secList:
                ...
                self.trackInstrumental.append(mido.Message('note_on', note=section.intensity%5, velocity=self.volume, time=0))
                self.trackInstrumental.append(mido.Message('note_off', note=section.intensity%5, velocity=self.volume, time=1))
            '''
            for section in secList:
                distanceFromCenter = math.sqrt(math.pow(section.x_pos, 2) + math.pow(section.y_pos, 2))
    ...
    def SynthConvert(self):
        ...
        while len(ListOfSect) != 0:
            for chanel in range(1, self.chanelCount):
                ...
                currentFreq = self.freq_hz/chanel
